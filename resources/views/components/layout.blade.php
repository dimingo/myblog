    ...
    <section class="px-6 py-8">
            ...
            <div class="mt-8 md:mt-0 flex items-center">
                ...
                <x-dropdown>
                    <x-slot name="activator">
                        <button class="text-xs  p-2 font-bold uppercase">Welcome, {{auth()->user()->name}}</button>
                    </x-slot>
                    <x-dropdown-item href="/admin/dashboard" :active="request()->is('/admin/dashboard')">Dashboard</x-dropdown-item>
                    <x-dropdown-item href="/admin/posts/create" :active="request()->is('/admin/posts/create')">New Post</x-dropdown-item>
                    <x-dropdown-item href="/admin/posts/create" :active="request()->is('/admin/posts/create')" @click.prevent="document.querySelector('#logout-form').submit()">Log Out</x-dropdown-item>
                    <form method="POST" action='/logout' id="logout-form" hidden>
                        @csrf
                        <button type="submit" class="text-xs ml-2 font-bold text-blue-500 Capitalize">Logout</button>
                    </form>
                </x-dropdown>

                @else

                <a href='/register' class="text-xs ml-2 font-bold Capitalize">Register</a>
        ...
        <footer class="bg-gray-100 border border-black border-opacity-5 rounded-xl text-center py-16 px-10 mt-16" id="newsletter">
            ...
            <div class="mt-10">
                ...
                <div class="text-sm text-center text-red-500 p-2 col-start-5">
                    @error('email')
